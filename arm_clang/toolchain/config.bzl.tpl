...
def wrapper_path(ctx, tool):
    return "{}/bin/{}{}".format("%{armclang_root_path}", tool, "%{wrapper_ext}")
...
def _config_c_cpp(ctx, _action_configs, features):
    ...
    features.append(new_feature(
        ...
        c_flags + [
            "-fdebug-prefix-map=%{compile_root}=.",
        ],
    ))
...
def _impl(ctx):
    ...
    # Disable user flags to control the order
    features.append(feature(
    ...
    builtin_includes = ["%{armclang_root_path}/include"]
