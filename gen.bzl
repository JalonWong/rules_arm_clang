...
load("@arm_clang_//toolchain:gen.bzl", "armclang_gen_asm", "armclang_gen_bin", "armclang_gen_hex")
...
def _impl_gen(ctx, filetype):
    ...
    if "bin" == filetype:
        armclang_gen_bin(ctx, in_file, out_file, input_other)
    elif "hex" == filetype:
        armclang_gen_hex(ctx, in_file, out_file, input_other)
    elif "asm" == filetype:
        armclang_gen_asm(ctx, in_file, out_file)
